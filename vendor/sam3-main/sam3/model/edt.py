...
try:
    ...
    # Triton is a CUDA-only dependency and is unavailable on some platforms
    # (e.g. macOS). The edt_kernel below is only ever launched on CUDA
    # (edt_triton asserts data.is_cuda), so shim the decorator and the
    # `tl.constexpr` annotation just enough to let this module import. Nothing
    # here runs off-CUDA, so SAM3 image detection still works on CPU/MPS.
    class _TritonShim:  # noqa: N801
        @staticmethod
        def jit(fn=None, **kwargs):
            return fn if callable(fn) else (lambda f: f)

    class _TLShim:  # noqa: N801
        constexpr = None

        def __getattr__(self, name):
            return None

    triton = _TritonShim()  # type: ignore[assignment]
    tl = _TLShim()  # type: ignore[assignment]
...
def edt_triton(data: torch.Tensor):
    ...
    assert data.dim() == 3
    if not data.is_cuda:
        # CPU/MPS fallback (no Triton/CUDA): OpenCV's L2 distance transform is
        # exactly the kernel's intent — distance to the nearest zero pixel —
        # computed per batch element.
        import cv2
        import numpy as np

        arr = data.detach().to("cpu")
        out = []
        for b in range(arr.shape[0]):
            mask = (arr[b].numpy() != 0).astype(np.uint8)
            dt = cv2.distanceTransform(mask, cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
            out.append(torch.from_numpy(dt))
        return torch.stack(out, 0).to(device=data.device, dtype=torch.float32)
    assert data.is_cuda
    B, H, W = data.shape
