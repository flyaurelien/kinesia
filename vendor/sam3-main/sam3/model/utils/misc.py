...
def copy_data_to_device(data, device: torch.device, *args: Any, **kwargs: Any):
    ...
    # non_blocking host->device copies are unreliable on MPS: the source CPU
    # tensor can be read or freed before the async copy finishes, leaving
    # uninitialized garbage. Force a blocking copy on MPS.
