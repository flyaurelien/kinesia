...
class Sam3Image(torch.nn.Module):
    ...
    def _get_img_feats(self, backbone_out, img_ids):
        ...
        if img_ids.numel() > 1:
            # Only forward backbone on unique image ids to avoid repetitive
            # computation. torch.unique on int64 is unreliable on MPS, so run
            # the index bookkeeping on CPU.
            unique_ids = torch.unique(img_ids.cpu()).to(img_ids.device)
        else:
            unique_ids = img_ids
        ...
        # Next time we call this function, we want to remember which indices we
        # computed. Build the mapping on CPU (full()/index_put on int64 are
        # unreliable on MPS and can leave uninitialized garbage), then move it.
        id_mapping = torch.full((len(img_batch),), -1, dtype=torch.long)
        id_mapping[unique_ids.cpu()] = torch.arange(len(unique_ids))
        id_mapping = id_mapping.to(self.device)
        backbone_out = {
            **backbone_out,
