...
class Sam3Processor:
    ...
    def set_image_batch(self, images, state=None):
        """Run the (dominant) ViT backbone ONCE over a batch of frames. Grounding
        for each frame is then done per-image via ground_batch_image(state, prompt, i),
        which selects the image with find_stage.img_ids — the model already indexes
        batched features that way. All frames must share the same size."""
        if state is None:
            state = {}
        pil = [im if isinstance(im, PIL.Image.Image) else Image.fromarray(im) for im in images]
        if not pil:
            raise ValueError("set_image_batch needs at least one image")
        state["original_width"], state["original_height"] = pil[0].size
        batch = mx.stack([self.transform(im) for im in pil], axis=0)  # [N, C, H, W]
        state["backbone_out"] = self.model.backbone.call_image(batch)
        mx.eval(state["backbone_out"])
        state["_batch_size"] = len(pil)
        return state

    def ground_batch_image(self, state: Dict, prompt: str, img_index: int):
        """Ground one image (by index) of a batch produced by set_image_batch.
        The text prompt is encoded once and cached on the shared backbone_out."""
        if "backbone_out" not in state:
            raise ValueError("You must call set_image_batch before ground_batch_image")
        ...
        self.find_stage = FindStage(
            img_ids=mx.array([img_index], dtype=mx.int64),
            text_ids=mx.array([0], dtype=mx.int64),
            input_boxes=None,
            input_boxes_mask=None,
            input_boxes_label=None,
    ...
    def _call_grounding(self, state: Dict):
        ...
        # Per-detection learned embedding (decoder query hidden state, 256-d),
        # aligned 1:1 with the kept boxes/scores — a strong re-identification
        # feature, far better than a colour histogram. None if unavailable.
        queries = outputs.get("queries")
        state["embeddings"] = (
            np.array(queries[0][indices]) if queries is not None and len(mask_np.nonzero()[0]) > 0
            else np.zeros((0, queries.shape[-1] if queries is not None else 256), dtype=np.float32)
        )
