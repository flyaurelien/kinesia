...
class HumanDetector:
    ...
    def sam3_run(self, img, det_cat_id: int = 0, bbox_thr: float = 0.5, text_prompt: str | None = None, **kwargs):
        # switch bgr to rgb
        img = img[:, :, ::-1].copy()
        img = Image.fromarray(img.astype('uint8'), 'RGB')
        inference_state = self.processor.set_image(img)
        # Prompt the model with the caller's text prompt (defaults to "person").
        prompt = (text_prompt or "person").strip() or "person"
